...
// 数组的泛型
let fibonacci2 = [1, 2, 3, 4];
let fibonacci3 = [1, 2, 3, 4];
//  类数组
function sum() {
    let args = arguments;
    let args2 = arguments; //  IArguments是ts定义好的类型，还有NodeList,HTMLCollection等
}
//  any在数组中的应用
let list = ['abc', 24, { website: 'www' }];
//  函数的类型
//  函数声明
function sum2(x, y) {
    return x + y;
...
function buildName(firstName, lastName = 'Tom') {
...
function reverse(x) {
    const transformedX = x.toString().split('').reverse().join();
    if (typeof x === 'number') {
 ...
 */
//# sourceMappingURL=knowledeg1.js.map